...
class ChunkDeltaBwdDhuSm90:
    ...
    def _prefetch_bv32(
        ...
        _cp_async_mn_sw64_32x64(
            tidx,
            (sDo_ptr + v_stage_off).toint(),
            do_ptr + base_v,
            v_row_stride_bytes,
        )
        _cp_async_dv_bridge_64x32(
            tidx,
            (sDvPref_ptr + dv_stage_off).toint(),
            dv_ptr + base_v,
            v_row_stride_bytes,
        )
        cute.arch.cp_async_commit_group()
    ...
    def kernel(
        ...
        if cutlass.const_expr(self.BV == 32):
            s_k_stage_outer = cute.append(s_k_layout.outer, cute.make_layout((3,), stride=(cute.cosize(s_k_layout.outer),)))
            s_qw_stage_outer = cute.append(s_qw_layout.outer, cute.make_layout((3,), stride=(cute.cosize(s_qw_layout.outer),)))
            s_vt_stage_outer = cute.append(s_vt_layout.outer, cute.make_layout((3,), stride=(cute.cosize(s_vt_layout.outer),)))
        ...
        if cutlass.const_expr(self.BV == 32 or self.H_static < 64):
        ...
        for iter_idx in cutlass.range(NT):
            ...
            t0 = chunk * BT
            if cutlass.const_expr(self.BV == 64 and self.H_static == 64):
                base_k = ((b * T + t0) * self.H_static + h) * K_DIM
            base_v = ((b * T + t0) * self.H_static + h) * V_DIM + v_base
            ...
            if cutlass.const_expr(self.BV == 64):
                if cutlass.const_expr(self.H_static < 64):
                    dv_pref_ptr = sDvPrefRaw.iterator + (iter_idx % 2) * (BT * self.BV)
                else:
                    dv_pref_ptr = sDvPrefRaw.iterator
